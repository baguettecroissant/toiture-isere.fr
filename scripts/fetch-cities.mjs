...
async function fetchCities() {
  const url = 'https://geo.api.gouv.fr/departements/38/communes?fields=nom,code,population,codesPostaux,centre';
  
  console.log('Fetching Isere (38) communes from Geo API...');
  try {
    ...
    // Process and filter communes in Isere (38) with population >= 2000
    const processed = data
      .map(c => {
        let lat = 45.1885; // Grenoble lat
        let lng = 5.7245;  // Grenoble lng
      ...
      .filter(c => c.codePostal !== '' && c.codePostal.startsWith('38') && c.population >= 2000)
